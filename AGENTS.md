...
- **AI**: Codex API `Codex-sonnet-4-20250514` — Vision으로 사진 분석 후 귀여운 코멘트 생성
- **배포**: 앱인토스 콘솔 (웹앱)
...
3. Codex Vision → 귀여운 AI 코멘트 자동 생성 ("콩이 한마디")
...
│   └── Codex.js         # Codex API 호출 (Vision 코멘트 생성)
├── hooks/
│   ├── useAuth.js
│   ├── usePosts.js
│   └── useStreak.js
...
> ⚠️ Codex API 키는 클라이언트에 노출되지 않도록 Supabase Edge Function으로 감싸는 걸 권장.
